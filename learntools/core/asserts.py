...
def assert_equal(actual, expected, name, failure_factory=None):
    ...
    if isinstance(expected, float):
        ...
        check = np.array_equal(actual, expected, equal_nan = True) # equal_nan is True by default. # Modified by GBM
    
    ##elif isinstance(actual, pd.Index) or isinstance(expected, pd.Index): # Modified by GBM
    ##    check = (actual & expected).all()

    ## NOTICE: this may or may not work with objects other than strings, maybe use an equals function?
    ##elif isinstance(actual, pd.Index) or isinstance(expected, pd.Index):
